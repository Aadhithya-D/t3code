...
export interface AcpSessionRuntimeOptions {
  ...
  /**
   * ACP authentication is optional. Agents such as Kiro authenticate outside
   * the protocol and do not implement the `authenticate` method.
   */
  ...
  Effect.gen(function* () {
    ...
    const cancel = Effect.gen(function* () {
      ...
      if (options.cancelBehavior !== "wait-for-prompt") {
        // Notify the agent first. If we interrupt the local RPC fiber before
        // `session/cancel`, an uninterruptible `session/prompt` (Kiro while
        // tools/subagents run) never sees the cancel and Stop deadlocks.
